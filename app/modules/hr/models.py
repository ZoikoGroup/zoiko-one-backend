...
class CaseInsensitiveEnum(TypeDecorator):
    ...
    Stores enum values in database as uppercase (the .name of each member)
    but accepts both uppercase and lowercase values when reading from DB.
    """
    impl = VARCHAR
    cache_ok = True
    ...
    def process_bind_param(self, value, dialect):
        """Convert Python enum to database value (uppercase name)."""
        if value is None:
            return None
