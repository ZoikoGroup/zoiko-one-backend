...
def list_runs(db: Session = Depends(get_db), _=Depends(get_current_user)):
    return service.get_payroll_runs(db)
...
def list_items(run_id: int, db: Session = Depends(get_db), _=Depends(get_current_user)):
    return service.get_payslips_for_run(db, run_id)
